...
impl IrcParser {
    ...
    /// Parse a complete IRC message into constituents
    pub fn parse_message<'a>(&'a self, input: &'a str) -> Result<IrcMessage<'a>, ParseError> {
        let input = input.trim_end_matches('\n').trim_end_matches('\r');
        let mut lexer = Lexer::new(input);
        self.parse_irc_message(&mut lexer)
    }

    pub fn extract_chat_data<'a>(
        &'a self,
        message: IrcMessage<'a>,
    ) -> Result<ChatData<'a>, ParseError> {
    ...
    pub fn parse_source<'a>(&'a self, lexer: &mut Lexer<'a>) -> Result<IrcSource<'a>, ParseError> {
        ...
        if nick_end == source_str.len() {
            ...
            //
            // I don't think this should be triggered but just in case...
            return Ok(IrcSource {
                nick,
                user: None,
                host: None,
    ...
    pub fn parse_params<'a>(&self, lexer: &mut Lexer<'a>) -> Vec<&'a str> {
...
mod tests {
    ...
    fn test_parse_privmsg() {
        ...
        let message = parser.parse_message(input).unwrap();
