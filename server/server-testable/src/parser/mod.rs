...
use crate::{parser::lexer::Lexer, ws::client::WsClientResult};
...
pub trait Parser: Send + Sync + fmt::Debug {
    fn parse<'a>(&'a self, raw_msg: &'a str) -> WsClientResult<IrcMessage<'a>>;
    fn extract_chat_data<'a>(&'a self, message: &IrcMessage<'a>) -> WsClientResult<ChatData<'a>>;
    fn extract_channel<'a>(&self, message: &IrcMessage<'a>) -> WsClientResult<&'a str>;
}
    ...
    fn extract_chat_data<'a>(&'a self, message: &IrcMessage<'a>) -> WsClientResult<ChatData<'a>> {
        if message.command != "PRIVMSG" {
            if message.command == "NOTICE" {
                println!(
                    "Received 'NOTICE' command (channel '{}'): {:#?}",
